        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#testimonial-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i>
                        {{ __('Add Item') }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                <td>{{ $item->user_name ?? '' }}</i></td>
                                <td>{{ $item->user_type ?? '' }}</i></td>
                                <td><img src="{{ get_image($item->user_image ?? '', 'site-section') }}" alt=""
                                        srcset="" width="80"></td>
                                <td>{{ substr($item->language->$system_default_lang->details, 0, 30) . '...' ?? '' }}</i></td>
                                <td>{{ $item->icon_show ?? '' }}</i></td>
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                ...
                                <div class="tab-pane @if (get_default_language_code() == language_const()::NOT_REMOVABLE) fade show active @endif"
                                    ...
                                    <div class="form-group">
                                        @include('admin.components.form.textarea', [
                                            ...
                                            'value' => old(
                                                $default_lang_code . '_details_edit',
                                                $data->value->language->$default_lang_code->details ?? ''),
                                        ])
                                    </div>
                                </div>
                                ...
                                <div class="col-xl-12 col-lg-12 form-group">
                                    @include('admin.components.form.input-file', [
                                        'label' => __('User Image') . '*',
                                        'name' => 'user_image_edit',
                                        'class' => 'file-holder',
                                        'old_files_path' => files_asset_path('site-section'),
                                        'old_files' => $data->value->user_image ?? '',
                                    ])
                                </div>
                                <div class="form-group">
                                    @include('admin.components.form.input', [
                                        'label' => __('User Name') . '*',
                                        'name' => 'user_name_edit',
                                        'value' => old('user_name_edit', $data->value->user_name ?? ''),
                                        'class' => 'form--control',
                                    ])
                                </div>
                                <div class="form-group">
                                    @include('admin.components.form.input', [
                                        'label' => __('User Type') . '*',
                                        'name' => 'user_type_edit',
                                        'value' => old('user_type_edit', $data->value->user_type ?? ''),
                                        'class' => 'form--control',
                                    ])
                                </div>
                                <div class="form-group">
                                    @include('admin.components.form.input', [
                                        'label' => __('Icon Show') . '*',
                                        'name' => 'icon_show_edit',
                                        'value' => old('icon_show_edit', $data->value->icon_show ?? ''),
                                        'class' => 'form--control',
                                    ])
                                </div>
                            </div>
...
@push('script')
    ...
    <script>
        ...
        $(".edit-modal-button").click(function() {
            ...
            editModal.find("input[name=user_image_edit]").attr("data-preview-name", oldData.user_image);
            editModal.find("input[name=icon_show_edit]").val(oldData.icon_show);
            editModal.find("input[name=user_type_edit]").val(oldData.user_type);
            editModal.find("input[name=user_name_edit]").val(oldData.user_name);
            ...
            fileHolderPreviewReInit("#testimonial-edit input[name=user_image_edit]");
